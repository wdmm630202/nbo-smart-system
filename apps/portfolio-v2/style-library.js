...
const validVisibility = new Set(["published", "hidden"]);
  ...
  const assignments = Object.fromEntries(normalizedCatalog.styles.map((style) => {
    ...
    const slots = assignment.slots.map((slot, index) => {
      ...
      if (slot.source === "upload" && slotUpdatedAt === null) {
        throw new Error(`风格 ${style.id} 照片位 ${index + 1} upload 来源必须设置更新时间`);
      }
      if (slot.source !== "upload" && slotUpdatedAt !== null) {
...
export function buildStyleLibrary({ catalog, assignments, assets, requireExplicitSlotIds = false }) {
  ...
  return {
    ...
    featuredStyleIds: normalizedCatalog.featuredStyleIds.filter((styleId) =>
      publishedStyles.some(({ id }) => id === styleId)),
    ...
    slots,
    counts: {
      styles: publishedStyles.length,
      publishedStyles: publishedStyles.length,
      indoor: publishedStyles.filter(({ scene }) => scene === "indoor").length,
      outdoor: publishedStyles.filter(({ scene }) => scene === "outdoor").length,
      slots: slots.length,
