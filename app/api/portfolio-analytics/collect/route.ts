...
type SessionSnapshot = {
  ...
  briefOpens?: unknown;
  briefCopies?: unknown;
  filterChanges?: unknown;
  lcpMs?: unknown;
  interactionMs?: unknown;
  clsMilli?: unknown;
};
...
function integerValue(value: unknown, min: number, max: number, fallback = 0) {
...
function scoreIntent(session: Required<SessionSnapshot>) {
  let score = 0;
...
export async function POST(request: Request) {
  ...
  if (declaredLength > 24_000) return Response.json({ error: "payload_too_large" }, { status: 413, headers: corsHeaders(origin!) });

  try {
    const raw = await request.text();
    if (raw.length > 24_000) return Response.json({ error: "payload_too_large" }, { status: 413, headers: corsHeaders(origin!) });
    const payload = JSON.parse(raw) as { session?: SessionSnapshot; events?: PortfolioEvent[] };
    const source = payload.session;
    const now = new Date().toISOString();
    const sessionId = stringValue(source?.id, 64);
    if (!/^[a-f0-9-]{20,64}$/i.test(sessionId)) {
      return Response.json({ error: "invalid_session" }, { status: 400, headers: corsHeaders(origin!) });
    }
    ...
    const session = {
      ...
      device: stringValue(source?.device, 16, "unknown"),
      source: stringValue(source?.source, 40, "direct") || "direct",
      medium: stringValue(source?.medium, 40, "none") || "none",
      campaign: stringValue(source?.campaign, 80),
      referrerDomain: stringValue(source?.referrerDomain, 120),
      landingPath: stringValue(source?.landingPath, 120, "/p/") || "/p/",
    ...
    for (const event of events) {
      const eventType = stringValue(event.type, 32);
      const eventId = stringValue(event.id, 80);
      ...
      `).bind(
        ...
        session.id,
        eventType,
        stringValue(event.targetId, 64),
        stringValue(event.targetLabel, 80),
        stringValue(event.theme, 64),
        stringValue(event.scene, 32),
    ...
    return Response.json({ ok: true }, { status: 202, headers: corsHeaders(origin!) });
  } catch (error) {
    console.error("portfolio analytics collect failed", error);
    return Response.json({ error: "collect_failed" }, { status: 500, headers: corsHeaders(origin!) });
  }
}
