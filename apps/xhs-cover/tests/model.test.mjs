...
test('高端美业参考版使用克制文案与宋体层级', () => {
  ...
  assert.deepEqual(model.titleTypography(), {
    ...
    textAlign: 'left',
    accentColor: 'rgba(210,210,210,.72)',
  });
});

test('不同字体按实际笔画边缘进行光学左对齐', () => {
  assert.equal(typeof model.opticalLeftX, 'function');
  assert.equal(model.opticalLeftX(64, -2), 62);
  assert.equal(model.opticalLeftX(64, 1.5), 65.5);
});
